...
ACTION_DATA = {"method": "default-action-post", "enabled": True}
REQUESTS = []
...
def write_runtime_td(test_dir: Path, href: str) -> Path:
    ...
    td["actions"]["submitConfig"]["forms"][0]["href"] = href
...
async def main():
    ...
    try:
        await thing.invoke_action("submitConfig", ACTION_DATA)
        assert len(REQUESTS) == 1
        request = REQUESTS[0]
        assert request["method"] == "POST", f"Expected POST, got {request['method']}"
        assert request["path"] == "/config"
        assert request["content_type"] == "application/json"
        assert json.loads(request["body"].decode("utf-8")) == ACTION_DATA
    finally:
        await thing.cleanup()
        server.shutdown()
        server.server_close()
